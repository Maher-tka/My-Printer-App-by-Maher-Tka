...
const ObjectRow = memo(function ObjectRow({
  ...
  return (
    <div
      className={`flex items-center gap-1 rounded-md border px-1 py-1.5 text-sm ${selected ? 'border-primary bg-primary/5' : 'bg-muted/20'} ${isKey ? 'ring-2 ring-amber-300' : ''}`}
    >
      <Button
        type="button"
        size="icon"
      ...
      <Button
        ...
        aria-label={`Toggle ${object.name} lock`}
      >
      ...
      >
        <span className="block truncate font-medium">{object.name}</span>
        <span className="block truncate text-[11px] text-muted-foreground">
          {object.role} · {object.id.slice(-6)}
        </span>
      </button>
      <Button
        type="button"
        ...
        className="size-7"
        disabled={!selected}
        onClick={() => onSetKey(isKey ? undefined : object.id)}
