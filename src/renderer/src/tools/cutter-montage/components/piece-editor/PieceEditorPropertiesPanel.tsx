      ...
      <Panel title="Piece">
        <div className="grid grid-cols-2 gap-2">
          <NumberField
            ...
            value={piece.widthCm}
            step={0.1}
            onChange={(value) => onPieceSizeChange(value, piece.heightCm, 'width')}
          />
          <NumberField
            label="Height cm"
            value={piece.heightCm}
            step={0.1}
            onChange={(value) => onPieceSizeChange(piece.widthCm, value, 'height')}
          />
          <NumberField
            label="Quantity"
        ...
        {selectedObject ? (
          <div className="grid grid-cols-2 gap-2">
            <NumberField
              ...
              step={0.1}
              onChange={(xCm) => onObjectTransformChange(selectedObject.id, { xCm })}
            />
            <NumberField
              label="Y"
              value={selectedObject.transform.yCm}
              step={0.1}
              onChange={(yCm) => onObjectTransformChange(selectedObject.id, { yCm })}
            />
            <NumberField
              label="Width"
              ...
              step={0.1}
              onChange={(widthCm) =>
                onObjectTransformChange(selectedObject.id, { widthCm: Math.max(widthCm, 0.2) })
              }
            />
            <NumberField
              label="Height"
              ...
              step={0.1}
              onChange={(heightCm) =>
                onObjectTransformChange(selectedObject.id, { heightCm: Math.max(heightCm, 0.2) })
              }
            />
            <NumberField
              label="Rotation"
              ...
              step={1}
              onChange={(rotation) => onObjectTransformChange(selectedObject.id, { rotation })}
            />
          </div>
        ) : (
