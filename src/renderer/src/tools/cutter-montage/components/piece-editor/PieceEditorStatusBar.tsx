...
export function PieceEditorStatusBar({ piece }: { piece: PiecePreset }): JSX.Element {
  ...
  return (
    <div className="mt-3 flex flex-wrap items-center gap-2 rounded-md border bg-card px-3 py-2 text-xs">
      ...
      <Status
        active={cutline}
        label={cutline ? 'CutContour active' : 'CutContour hidden'}
        tone="pink"
      />
      <Status
        active={piece.groupLinked}
        label={piece.groupLinked ? 'Linked / grouped' : 'Independent objects'}
        tone="violet"
      />
      {piece.keyObjectId ? (
        <Status
          active
          label={`Key: ${piece.objects.find((object) => object.id === piece.keyObjectId)?.name ?? 'Object'}`}
          tone="amber"
        />
      ) : null}
    </div>
