...
export function usePieceEditorShortcuts(actions: PieceEditorShortcutActions): (
  event: ReactKeyboardEvent<HTMLElement>
) => void {
  return useCallback((event: ReactKeyboardEvent<HTMLElement>): void => {
    ...
    else if (event.key === '+' || event.key === '=') actions.setZoom((value) => Math.min(value + 0.15, 2.5))
